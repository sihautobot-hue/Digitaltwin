...
def generate_scientific_report(benchmark_data: dict, eval_data: dict):
    ...
    lines = [
        "# Model 6 (Version 3) Scientific Report: Day-Ahead Operational Risk Forecasting",
        "",
        ...
        "",
        f"Among four benchmarked tree ensemble architectures evaluated on strictly partitioned chronological splits (Train: 2003-2019, Val: 2020-2021, Hold-out Test: 2022), **{winner}** achieved superior validation performance and was selected as the operational model.",
        "",
        "---",
        "",
        ...
        bench_table,
        "",
        f"> **Decision:** **{winner}** achieved the highest Validation PR-AUC and is selected as the winning production model.",
        "",
        "---",
        "",
        f"## 4. Chronological Validation & Test Performance (2022 Hold-Out)",
        "",
        f"Optimal probability threshold was calibrated on the validation set to **{opt_thresh:.4f}** (Validation F1 = {val_f1_opt:.4f}) and applied unchanged to the 2022 hold-out test set.",
        "",
        "```",
        f"+---------------------------------------------------------------------------------------------------+",
        f"| HOLD-OUT TEST RESULTS (2022 CHRONOLOGICAL EVALUATION @ THRESHOLD = {opt_thresh:.4f})              |",
        f"+-----------------------------------+---------------------------------------------------------------+",
        f"| Accuracy                          | {overall['accuracy']:.4f}                                     |",
        f"| Precision                         | {overall['precision']:.4f}                                    |",
        f"| Recall (Sensitivity)              | {overall['recall']:.4f}                                       |",
        f"| F1 Score                          | {overall['f1']:.4f}                                           |",
        f"| PR-AUC (Average Precision)        | {overall['pr_auc']:.4f}                                       |",
        f"| ROC-AUC                           | {overall['roc_auc']:.4f}                                      |",
        f"| Balanced Accuracy                 | {overall['balanced_accuracy']:.4f}                            |",
        f"| Matthews Correlation Coeff (MCC)  | {overall['mcc']:.4f}                                          |",
        f"| Brier Score Loss                  | {overall['brier_score']:.4f}                                  |",
        f"| Expected Calibration Error (ECE)  | {overall['expected_calibration_error']:.4f}                   |",
        f"| Confusion Matrix                  | TN={overall['confusion_matrix']['tn']}, FP={overall['confusion_matrix']['fp']}, FN={overall['confusion_matrix']['fn']}, TP={overall['confusion_matrix']['tp']} |",
        f"+-----------------------------------+---------------------------------------------------------------+",
        "```",
        "",
        "---",
        ...
        loso_3_rows,
        "",
        f"**Deduplicated 3-Fold Mean PR-AUC:** {np.mean(loso_3_pr):.4f} (+/- {np.std(loso_3_pr):.4f}) | **Mean F1:** {np.mean(loso_3_f1):.4f}",
        "",
        "---",
        "",
