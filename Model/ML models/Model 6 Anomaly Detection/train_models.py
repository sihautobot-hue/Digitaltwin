...
from config import MODELS_DIR, RESULTS_DIR, TARGET_NAME, FEATURE_COLS, MODEL_PARAMS
...
def score_split(model, X, y, scaler, tag):
    ...
    logger.info('[%s] Acc=%.4f Prec=%.4f Rec=%.4f F1=%.4f ROC-AUC=%.4f PR-AUC=%.4f Brier=%.4f',
        tag, acc, prec, rec, f1, auc, pr_auc, brier)
