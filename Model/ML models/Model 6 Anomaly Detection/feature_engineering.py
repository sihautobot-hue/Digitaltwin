# feature_engineering.py -- Model 6 V3
import os, sys, hashlib, logging, warnings
...
def engineer_features(df):
    ...
    for col in ANOMALY_SOURCE_COLS:
        if col in df.columns:
            anom = anom | df[col].fillna(False).astype(bool).values
    ...
    shifted_recv = df["received_today"].shift(1) > 0
    df["days_since_last_delivery"]      = days_since_event(shifted_recv)
