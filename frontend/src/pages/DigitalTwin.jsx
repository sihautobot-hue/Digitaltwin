...
export const DigitalTwin = () => {
  const { stationData, activeStation } = useStationData();
  ...
  const stationBaseLabel = activeStation === 'bharati' ? 'Larsemann Hills Base' : 'Schirmacher Oasis Base';
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white tracking-tight font-display flex items-center gap-2">
              ...
              {station?.name || 'Maitri'} – 2D Station Physical Digital Twin
            </h1>
          ...
          <p className="text-xs font-scada-mono text-slate-500 dark:text-zinc-400 mt-1">
            REAL-TIME PHYSICAL TELEMETRY MAPPING | 7 REINFORCED MODULES | {stationBaseLabel.toUpperCase()}
          </p>
        </div>

        {/* Filter Toolbar */}
        <div className="flex items-center gap-2 bg-white dark:bg-[#1E1E1E] p-1 rounded-lg border border-slate-200 dark:border-[#2A2A2A] text-xs font-scada-mono shadow-sm">
      ...
      <div>
        <h3 className="text-sm font-bold text-slate-900 dark:text-white tracking-wide uppercase font-scada-mono mb-3 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {filteredModules.map((mod) => (
            <div
              ...
              className="bg-white dark:bg-[#1E1E1E] border border-slate-200 dark:border-[#2A2A2A] hover:border-cyan-500/60 rounded-xl p-4 cursor-pointer transition flex flex-col justify-between group shadow-sm"
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-scada-mono font-bold text-cyan-600 dark:text-cyan-400 group-hover:text-cyan-500">
                    {mod.code}
                  </span>
                ...
                <div className="space-y-1.5 text-xs font-scada-mono text-slate-700 dark:text-zinc-300 pt-2 border-t border-slate-100 dark:border-[#262626]">
                  {mod.tempC !== undefined && (
                    <div className="flex justify-between">
                      <span className="text-slate-400 dark:text-zinc-500">TEMP:</span>
                      <span className="font-bold text-slate-900 dark:text-white">{mod.tempC}°C</span>
                    </div>
                  )}
                  {mod.powerDrawKw !== undefined && (
                    <div className="flex justify-between">
                      <span className="text-slate-400 dark:text-zinc-500">POWER DRAW:</span>
                      <span className="font-bold text-cyan-600 dark:text-cyan-400">{mod.powerDrawKw} kW</span>
                    </div>
                  )}
                  {mod.oxygenPercent !== undefined && (
                    <div className="flex justify-between">
                      <span className="text-slate-400 dark:text-zinc-500">OXYGEN (O2):</span>
                      <span className="font-bold text-green-600 dark:text-green-400">{mod.oxygenPercent}%</span>
                    </div>
                  )}
                  {mod.occupancy !== undefined && (
                    <div className="flex justify-between">
                      <span className="text-slate-400 dark:text-zinc-500">CREW OCCUPANCY:</span>
                      <span className="font-bold text-slate-800 dark:text-zinc-200">{mod.occupancy}</span>
                    </div>
                  )}
                </div>
              </div>

              <div className="mt-3 pt-2 border-t border-slate-100 dark:border-[#262626] flex items-center justify-between text-[10px] font-scada-mono text-slate-400 dark:text-zinc-500">
                <span>STATUS: {mod.status}</span>
                <span className="text-cyan-600 dark:text-cyan-400 group-hover:underline flex items-center gap-0.5 font-bold">
                  INSPECT →
                </span>
              </div>
            </div>
