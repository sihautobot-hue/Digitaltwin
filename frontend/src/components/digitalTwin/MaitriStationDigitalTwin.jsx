...
export const MaitriStationDigitalTwin = ({ 
  ...
  return (
    <div className="bg-[#181D26] border border-[#232B3B] rounded-xl overflow-hidden shadow-2xl flex flex-col relative">
      {/* Top Header Controls */}
      <div className="px-4 py-3 bg-[#131722]/80 border-b border-[#232B3B] flex items-center justify-between z-20 backdrop-blur-sm">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-bold text-white tracking-wide">
            Maitri Station – 2D Digital Twin View
          </h2>
        </div>
        ...
        <div className="flex items-center gap-4 text-xs font-sans">
          {/* Labels Toggle */}
          <div className="flex items-center gap-2">
            <span className="text-zinc-400 text-xs font-medium">Labels</span>
            <button
              onClick={() => setShowLabels(!showLabels)}
              className={`w-9 h-5 rounded-full transition-colors relative flex items-center p-0.5 ${
                showLabels ? 'bg-blue-600' : 'bg-zinc-700'
              }`}
            >
              <div
          ...
          {/* View Dropdown */}
          <div className="flex items-center gap-1 bg-[#1A2232] border border-[#2B374E] px-2.5 py-1 rounded-md text-zinc-200">
            <span className="text-zinc-400 text-xs">View:</span>
            <span className="font-semibold text-xs text-white">{viewMode}</span>
      ...
      <div className="relative w-full aspect-[16/9] min-h-[380px] bg-[#0E131F] overflow-hidden select-none group">
        ...
        <img
          ...
          alt="Maitri Research Station 2D Digital Twin"
              ...
              {showLabels && (
                <div
                  className={`mt-1.5 min-w-[130px] rounded-lg px-2.5 py-1.5 shadow-2xl border ${
                    isSelected ? 'border-cyan-400 bg-[#162032]' : `${style.border} ${style.bg}`
                  } backdrop-blur-md cursor-pointer transition-all duration-200 hover:scale-105 ${
                    isHovered || isSelected ? 'ring-1 ring-cyan-400' : ''
                  }`}
                >
                  <div className="text-[11px] font-bold text-white whitespace-nowrap leading-tight">
                    {mod.name}
                  </div>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className={`w-1.5 h-1.5 rounded-full ${style.dot}`} />
                    <span className={`text-[10px] font-medium ${style.text} whitespace-nowrap`}>
                      {style.badgeText}
                    </span>
                  </div>
                </div>
      ...
      {/* Bottom Legend Bar */}
      <div className="px-4 py-2.5 bg-[#131722]/90 border-t border-[#232B3B] flex flex-wrap items-center justify-between gap-3 text-xs z-20">
        <div className="flex items-center gap-4 text-xs font-sans">
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-green-500" />
            <span className="text-zinc-300 text-xs">Normal</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-yellow-500" />
            <span className="text-zinc-300 text-xs">Warning</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-red-500" />
            <span className="text-zinc-300 text-xs">Critical</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-zinc-500" />
            <span className="text-zinc-300 text-xs">Offline</span>
          </div>
        </div>

        <div className="text-[11px] text-zinc-400 font-sans">
          Click on any module to see detailed live data
        </div>
      </div>
    </div>
  );
